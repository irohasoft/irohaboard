...
# 管理者アカウントのパスワードの復旧方法
# 1. UPDATEの前の先頭の#を削除し、復旧したい管理者のログインIDとパスワードを設定します。
# 2. ファイルを保存後、ブラウザで/update を実行します。
# 3. #を元に戻し、ファイルを保存します。
#UPDATE ib_users SET `password` = SHA1(CONCAT('397110e45242a23e5802e78f4eec95a7bd39e0f0', '新しいパスワード')) WHERE username = '復旧したい管理者のログインID';

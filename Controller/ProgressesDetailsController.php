...
class ProgressesDetailsController extends AppController
{
    ...
    public function admin_index($progress_id)
    {
        ...
        $progress_info = $this->Progress->find("first", [
            "conditions" => [
    ...
    public function admin_edit($progress_id, $progress_detail_id = null)
    {
        ...
        if (
            $this->action == "edit" &&
            !$this->ProgressesDetail->exists($progress_detail_id)
        ) {
        ...
        if ($this->request->is(["post", "put"])) {
            ...
            if ($this->ProgressesDetail->save($request_data)) {
                ...
                $this->Flash->error(
                    __("The progress could not be saved. Please, try again.")
                );
            }
        } else {
            $options = [
                "conditions" => [
                    "ProgressesDetail." .
                    $this->ProgressesDetail->primaryKey => $progress_detail_id,
                ],
            ];
            $this->request->data = $this->ProgressesDetail->find(
                "first",
                $options
            );

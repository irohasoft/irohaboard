...
class InfosController extends AppController
{
        ...
        $this->Paginator->settings = [
            ...
            "joins" => [
                [
                    ...
                    "alias" => "InfoGroup",
                    "table" =>
                        '(SELECT ug.info_id, group_concat(g.title order by g.id SEPARATOR \', \') as group_title FROM ib_infos_groups ug INNER JOIN ib_groups g ON g.id = ug.group_id GROUP BY ug.info_id)',
    ...
    public function admin_edit($info_id = null)
    {
        ...
        if ($this->request->is(["post", "put"])) {
            ...
            if ($this->Info->save($this->request->data)) {
                ...
                $this->Flash->error(
                    __("The info could not be saved. Please, try again.")
                );
            }
        } else {
            $options = [
    ...
    public function admin_delete($info_id = null)
    {
        ...
        if ($this->Info->delete()) {
            ...
            $this->Flash->error(
                __("The info could not be deleted. Please, try again.")
            );
        }
        return $this->redirect([
            "action" => "index",

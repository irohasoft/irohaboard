...
class BenchmarkShell extends Shell
{
    ...
    public function main()
    {
        ...
        $this->out(
            String::insert(__d("debug_kit", "-> Testing :url"), compact("url"))
        );
        $this->out("");
        for ($i = 0; $i < $options["n"]; $i++) {
            if (
                floor($options["t"] - array_sum($times)) <= 0 ||
                $options["n"] <= 1
            ) {
                break;
            }
    ...
    protected function _results($times)
    {
        ...
        $this->out(
            String::insert(
                __d("debug_kit", "Total Requests made: :requests"),
                compact("requests")
            )
        );
        $this->out(
            String::insert(
                __d("debug_kit", "Total Time elapsed: :duration (seconds)"),
                compact("duration")
            )
        );
        ...
        $this->out(
            String::insert(__d("debug_kit", "Requests/Second: :rps req/sec"), [
        ...
        $this->out(
            String::insert(
                __d("debug_kit", "Average request time: :average-time seconds"),
                [
        ...
        $this->out(
            String::insert(
                __d(
                    "debug_kit",
                    "Standard deviation of average request time: :std-dev"
                ),
                [
        ...
        $this->out(
            String::insert(
                __d(
                    "debug_kit",
                    "Longest/shortest request: :longest sec/:shortest sec"
                ),
                [

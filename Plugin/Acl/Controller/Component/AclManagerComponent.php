...
{
    var $components = ["Auth", "Acl", "Acl.AclReflector", "Session"];
    ...
    public function initialize(Controller $controller)
    {
        ...
        $this->controllers_hash_file =
            CACHE . "persistent" . DS . "controllers_hashes.txt";
    }
    ...
    private function check_controller_hash_tmp_file()
    {
        if (is_writable(dirname($this->controllers_hash_file))) {
            ...
            $this->Session->setFlash(
                sprintf(
                    __d("acl", "the %s directory is not writable"),
                    dirname($this->controllers_hash_file)
                ),
                "flash_error",
                null,
                "plugin_acl"
    ...
    public function check_user_model_acts_as_acl_requester($model_classname)
    {
        ...
        if (!empty($behaviors) && array_key_exists("Acl", $behaviors)) {
            ...
            if ($acl_behavior == "requester") {
                ...
                is_array($acl_behavior) &&
                isset($acl_behavior["type"]) &&
                $acl_behavior["type"] == "requester"
            ) {
                return true;
            }
        }
    ...
    public function set_display_name($model_classname, $field_expression)
    {
        ...
        if (
            array_key_exists($field_expression, $schema) ||
            array_key_exists(
                str_replace($model_classname . ".", "", $field_expression),
                $schema
            ) ||
            ...
            $this->controller->{$model_classname}->virtualFields[
    ...
    function get_missing_acos()
    {
        ...
        $controllers_aco = $aco->find("first", [
            "fields" => ["id"],
            "conditions" => ["alias" => "controllers"],
            "recursive" => -1,
        ]);
            ...
            foreach ($aco_path_parts as $aco_path_part) {
                ...
                if (empty($node)) {
                    ...
                    if (isset($parent_node)) {
                        $parent_id = isset($parent_node)
                            ? $parent_node[0]["Aco"]["id"]
                            : null;
                    }
    ...
    public function controller_hash_file_is_out_of_sync()
    {
        if ($this->check_controller_hash_tmp_file()) {
             ...
             */
            $updated_controllers = array_keys(
    ...
    public function get_acos_to_prune()
    {
        ...
        foreach ($acos as $aco_node) {
            ...
            if (
                count($path_nodes) > 1 &&
                $path_nodes[0]["Aco"]["alias"] == "controllers"
            ) {
                $path = "";
                foreach ($path_nodes as $path_node) {
                    $path .= "/" . $path_node["Aco"]["alias"];
    ...
    public function save_permission($aro_nodes, $aco_path, $permission_type)
    {
        if (isset($aro_nodes[0])) {
            ...
            $specific_permission_right = $this->get_specific_permission_right(
                $aro_nodes[0],
                $aco_path
            );
            $inherited_permission_right = $this->get_first_parent_permission_right(
                $aro_nodes[0],
                $aco_path
            );
            ...
            if (!isset($inherited_permission_right) && count($aro_nodes) > 1) {
                 ...
                 */
                $specific_parent_aro_permission_right = $this->get_specific_permission_right(
                    $aro_nodes[1],
                    $aco_path
                );
            ...
            if (!isset($inherited_permission_right)) {
                ...
                if (
                    ...
                    $specific_permission_needed =
                        $inherited_permission_right != 1;
                } else {
                    $specific_permission_needed =
                        $inherited_permission_right == 1;
                }
            ...
            if ($specific_permission_needed) {
                ...
                if (!empty($aco_node)) {
                    ...
                    $specific_permission = $this->Acl->Aro->Permission->find(
                        "first",
                        [
                            "conditions" => [
                                "aro_id" => $aro_id,
                    ...
                    if (!empty($specific_permission)) {
                        if (
                            $this->Acl->Aro->Permission->delete([
                                "Permission.id" =>
                                    $specific_permission["Permission"]["id"],
                            ])
                        ) {
                            return true;
                        } else {
    ...
    private function get_specific_permission_right($aro_node, $aco_path)
    {
        $pk_name = "id";
        if (
            $aro_node["Aro"]["model"] == Configure::read("acl.aro.role.model")
        ) {
            $pk_name = $this->controller->_get_role_primary_key_name();
        } elseif (
            $aro_node["Aro"]["model"] == Configure::read("acl.aro.user.model")
        ) {
            $pk_name = $this->controller->_get_user_primary_key_name();
        }
        ...
        if (!empty($aco_node)) {
            ...
            $specific_permission = $this->Acl->Aro->Permission->find("first", [
                "conditions" => ["aro_id" => $aro_id, "aco_id" => $aco_id],
            ]);
            ...
            if (!empty($specific_permission)) {
                 ...
                 */
                $specific_permission_right = $this->Acl->check(
                    $aro_model_data,
                    $aco_path
                );
    ...
    private function get_first_parent_permission_right($aro_node, $aco_path)
    {
        ...
        while (
            strpos($aco_path, "/") !== false &&
            !isset($parent_permission_right)
        ) {
            ...
            if (!empty($parent_aco_node)) {
                ...
                $parent_permission = $this->Acl->Aro->Permission->find(
                    "first",
                    [
                        "conditions" => [
                            "aro_id" => $aro_id,
                ...
                if (!empty($parent_permission)) {
                     ...
                     */
                    $parent_permission_right = $this->Acl->check(
                        $aro_model_data,
                        $aco_path
                    );
    ...
    public function set_session_permissions()
    {
        if (!$this->Session->check("Alaxos.Acl.permissions")) {
            ...
            if (!empty($user)) {
                $user = [Configure::read("acl.aro.user.model") => $user];
                $permissions = [];

                foreach ($actions as $action) {
                    $aco_path = "controllers/" . $action;

                    $permissions[$aco_path] = $this->Acl->check(
                        $user,
                        $aco_path
                    );
                }

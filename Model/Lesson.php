...
class Lesson extends AppModel
{
    ...
    public function isDuringOnlineLessonHour($date_id)
    {
        ...
        foreach ($lessons as $lesson) {
            ...
            $half_hour_before_start = (int) strtotime(
                $lesson_date . " " . $lesson["Lesson"]["start"] . " -30 minute"
            );
            ...
            if ($half_hour_before_start <= $now_time && $now_time < $end) {
                return true;
            }
        }
        return false;
    ...
    public function checkLessonCode($input_code)
    {
        $today = date("Y-m-d");
        $data = $this->find("all", [
            "fields" => ["Lesson.id", "Lesson.code"],
            "conditions" => ["Date.date" => $today],
            "recursive" => 0,
        ]);
        foreach ($data as $datum) {
            if ($datum["Lesson"]["code"] == $input_code) {
                return true;
            }
        }
        return false;

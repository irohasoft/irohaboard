...
class InstallController extends AppController
{
        ...
        try {
            ...
            if (function_exists("apache_get_modules")) {
                ...
                if (!$this->__apache_module_loaded("mod_rewrite")) {
                    // エラー出力
                    $this->err_msg =
                        "Apache モジュール mod_rewrite がロードされていません";
                    $this->error();
                    $this->render("error");
                    return;
                }
                ...
                if (!$this->__apache_module_loaded("mod_headers")) {
                    // エラー出力
                    $this->err_msg =
                        "Apache モジュール mod_headers がロードされていません";
                    $this->error();
                    $this->render("error");
                    return;
                }
            ...
            if (!extension_loaded("mbstring")) {
                // エラー出力
                $this->err_msg =
                    "PHP モジュール mbstring がロードされていません";
                $this->error();
                $this->render("error");
                return;
            }
            ...
            if (!extension_loaded("pdo_mysql")) {
                // エラー出力
                $this->err_msg =
                    "PHP モジュール pdo_mysql がロードされていません";
                $this->error();
                $this->render("error");
                return;
            }
            ...
            $this->err_msg =
                "データベースへの接続に失敗しました。データベース設定ファイル(Config/database.php)をご確認ください。";
            $this->error();
            $this->render("error");
        }
    }
    ...
    private function __install()
    {
        ...
        if (count($err_statements) > 0) {
            $this->err_msg =
                "インストール実行中にエラーが発生しました。詳細はエラーログ(tmp/logs/error.log)をご確認ください。";

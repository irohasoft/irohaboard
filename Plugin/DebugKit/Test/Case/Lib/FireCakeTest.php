...
require_once CakePlugin::path("DebugKit") .
    "Test" .
    DS .
    "Case" .
    DS .
...
class FireCakeTestCase extends CakeTestCase
{
    ...
    public function testGetInstanceOverride()
    {
        ...
        $this->assertIsA(
            $instance,
            "TestFireCake",
            "Stored instance is not a copy of TestFireCake, test case is broken."
        );
    }
    ...
    {
        FireCake::setOptions(["includeLineNumbers" => false]);
        $this->assertEquals(
            $this->firecake->options["includeLineNumbers"],
            false
        );
    }
    ...
    public function testLog()
    {
        ...
        $this->assertTrue(
            isset($this->firecake->sentHeaders["X-Wf-Protocol-1"])
        );
        $this->assertTrue(
            isset($this->firecake->sentHeaders["X-Wf-1-Plugin-1"])
        );
        $this->assertTrue(
            isset($this->firecake->sentHeaders["X-Wf-1-Structure-1"])
        );
        $this->assertEquals($this->firecake->sentHeaders["X-Wf-1-Index"], 1);
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-1"],
            '26|[{"Type":"LOG"},"Testing"]|'
        );

        FireCake::log("Testing", "log-info");
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-2"],
            '45|[{"Type":"LOG","Label":"log-info"},"Testing"]|'
        );
    }
    ...
    public function testInfo()
    {
        ...
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-1"],
            '38|[{"Type":"INFO"},"I have information"]|'
        );

        FireCake::info("I have information", "info-label");
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-2"],
            '59|[{"Type":"INFO","Label":"info-label"},"I have information"]|'
        );
    }
    ...
    public function testWarn()
    {
        ...
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-1"],
            '29|[{"Type":"WARN"},"A Warning"]|'
        );

        FireCake::warn("A Warning", "Bzzz");
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-2"],
            '44|[{"Type":"WARN","Label":"Bzzz"},"A Warning"]|'
        );
    }
    ...
    public function testError()
    {
        ...
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-1"],
            '29|[{"Type":"ERROR"},"An error"]|'
        );

        FireCake::error("An error", "wonky");
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-2"],
            '45|[{"Type":"ERROR","Label":"wonky"},"An error"]|'
        );
    }
    ...
    {
        FireCake::dump("mydump", ["one" => 1, "two" => 2]);
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-2-1-1"],
            '28|{"mydump":{"one":1,"two":2}}|'
        );
        $this->assertTrue(
            isset($this->firecake->sentHeaders["X-Wf-1-Structure-2"])
        );
    }
    ...
    {
        $table[] = ["Col 1 Heading", "Col 2 Heading"];
        $table[] = ["Row 1 Col 1", "Row 1 Col 2"];
        $table[] = ["Row 2 Col 1", "Row 2 Col 2"];
        $table[] = ["Row 3 Col 1", "Row 3 Col 2"];
        FireCake::table("myTrace", $table);
        $expected =
            '162|[{"Type":"TABLE","Label":"myTrace"},[["Col 1 Heading","Col 2 Heading"],["Row 1 Col 1","Row 1 Col 2"],["Row 2 Col 1","Row 2 Col 2"],["Row 3 Col 1","Row 3 Col 2"]]]|';
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-1"],
            $expected
        );
    ...
    public function testStringEncode()
    {
        ...
        $this->firecake->setOptions(["maxArrayDepth" => 3]);
        $deep = [1 => [2 => [3]]];
        $result = $this->firecake->stringEncode($deep);
        $this->assertEquals($result, [1 => [2 => "** Max Array Depth (3) **"]]);
    }
    ...
    public function testStringEncodeObjects()
    {
        ...
        $this->assertEquals(
            $result["_defaultOptions"]["useNativeJsonEncode"],
            true
        );
        $this->assertEquals(
            $result["_encodedObjects"][0],
            "** Recursion (TestFireCake) **"
        );
    }
    ...
    public function testMultiLineOutput()
    {
        ...
        $this->assertGreaterThan(
            1,
            $this->firecake->sentHeaders["X-Wf-1-Index"]
        );
    ...
    {
        FireCake::setOptions(["includeLineNumbers" => true]);
    ...
    public function testGroup()
    {
        ...
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-1"],
            '63|[{"Collapsed":"true","Type":"GROUP_START","Label":"test"},null]|'
        );
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-3"],
    ...
    public function testFbParameterParsing()
    {
        ...
        $this->assertEquals(
            $this->firecake->sentHeaders["X-Wf-1-1-1-3"],
            '47|[{"Type":"WARN","Label":"Custom label"},"Test"]|'
        );

        $this->expectError("PHPUnit_Framework_Error");
        $this->assertFalse(
            FireCake::fb("Test", "Custom label", "warn", "more parameters")
        );
    ...
    public function testDetectClientExtension()
    {
        ...
        $_SERVER["HTTP_USER_AGENT"] =
            "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US; rv:1.9.0.4) Gecko/2008102920 Firefox/3.0.4 FirePHP/0.2.1";
        $this->assertTrue(FireCake::detectClientExtension());

        $_SERVER["HTTP_USER_AGENT"] =
            "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US; rv:1.9.0.4) Gecko/2008102920 Firefox/3.0.4 FirePHP/0.0.4";
    ...
    {
        FireCake::setOptions(["useNativeJsonEncode" => false]);
        $json = FireCake::jsonEncode(["one" => 1, "two" => 2]);
        $this->assertEquals($json, '{"one":1,"two":2}');

        $json = FireCake::jsonEncode([1, 2, 3]);

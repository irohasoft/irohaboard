...
DEBUGKIT.registerModules = function ($) {
  ...
  DEBUGKIT.Util.Event = (function () {
    ...
    var fixEvent = function (event) {
      ...
      if (event.pageX === null && event.clientX !== null) {
        ...
        event.pageX =
          event.clientX + (doc.scrollLeft || 0) - (doc.clientLeft || 0);
        event.pageY =
          event.clientY + (doc.scrollTop || 0) - (doc.clientTop || 0);
      }
      return event;
    };
  ...
  DEBUGKIT.Util.Request.prototype.onReadyStateChange = function () {
    ...
    if (
      ...
      (this.transport.status > 300 && this.transport.status < 400)
    ) {
      this.response = {
    ...
    return {
      ...
      makePanelDraggable: function (panel) {
        ...
        var mouseMoveHandler = function (event) {
          ...
          var newHeight =
            currentElement.data("startHeight") +
            (event.pageY - currentElement.data("startY"));
      ...
      activatePanel: function (id, unique) {
        if (this.panels[id] !== undefined && !this.panels[id].active) {
          ...
          var contentHeight =
            panel.content.find(".panel-content-data").height() + 70;
      ...
      makeNeatArray: function (lists) {
        lists.find("ul").hide().parent().addClass("expandable collapsed");
  ...
  DEBUGKIT.historyPanel = (function () {
    ...
    var restoreCurrentState = function () {
      ...
      $.each(toolbar.panels, function (panel, id) {
        ...
        var regionDivs = regionDiv.children();
        regionDivs.filter("div").show().end().filter(".panel-history").hide();
      });
    };

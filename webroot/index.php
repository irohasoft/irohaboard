...
if (version_compare(PHP_VERSION, "5.4.0") <= 0) {
    ...
    echo "ERROR-001 : iroha Board の動作には 5.4.0 以上が必要です。現在のバージョンは " .
        PHP_VERSION .
        " です。\n";
    exit();
}
...
if (!file_exists(ROOT)) {
    ...
    echo "現在の設定<br>" . "ROOT : " . ROOT . "<br>" . exit();
}
...
if (
    ...
    file_exists($vendorPath . DS . $dispatcher)
) {
...
if (PHP_SAPI === "cli-server") {
    if (
        $_SERVER["REQUEST_URI"] !== "/" &&
        file_exists(WWW_ROOT . $_SERVER["PHP_SELF"])
    ) {
        return false;
    }
...
if (!defined("CAKE_CORE_INCLUDE_PATH")) {
    if (function_exists("ini_set")) {
        ini_set(
            "include_path",
            ROOT . DS . "lib" . PATH_SEPARATOR . ini_get("include_path")
        );
    }
...
if (!empty($failed)) {
    trigger_error(
        "CakePHP core could not be found. Check the value of CAKE_CORE_INCLUDE_PATH in APP/webroot/index.php. It should point to the directory containing your " .
            DS .
            "cake core directory and your " .
            DS .

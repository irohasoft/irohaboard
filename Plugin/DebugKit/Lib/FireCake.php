...
class FireCake
{
    ...
    public static function detectClientExtension()
    {
        ...
        if (
            preg_match("/\sFirePHP\/([\.|\d]*)\s?/si", $ua, $match) &&
            version_compare($match[1], "0.0.6", ">=")
        ) {
            return true;
        }
        if (
            env("HTTP_X_FIREPHP_VERSION") &&
            version_compare(env("HTTP_X_FIREPHP_VERSION"), "0.6", ">=")
        ) {
            return true;
        }
        return false;
    ...
    public static function fb($message)
    {
        ...
        switch (count($args)) {
            ...
            default:
                trigger_error(
                    __d(
                        "debug_kit",
                        "Incorrect parameter count for FireCake::fb()"
                    ),
                    E_USER_WARNING
                );
                return false;
                ...
                for ($i = 0, $len = count($trace); $i < $len; $i++) {
                    ...
                    if ($selfCall) {
                        $meta["File"] = isset($trace[$i]["file"])
                            ? Debugger::trimPath($trace[$i]["file"])
                            : "";
        ...
        if ($type == $_this->_levels["dump"]) {
            ...
            $msg =
                "[" .
                $_this->jsonEncode($meta) .
                "," .
                $_this->jsonEncode($message, $skipFinalObjectEncode) .
                "]";
        }
        ...
        foreach ($lines as $i => $line) {
            ...
            $header =
                "X-Wf-1-" . $structureIndex . "-1-" . $_this->_messageIndex;
            if (count($lines) > 2) {
    ...
    protected static function _parseTrace($trace, $messageName)
    {
        ...
        for ($i = 0, $len = count($trace); $i < $len; $i++) {
            $keySet =
                isset($trace[$i]["class"]) && isset($trace[$i]["function"]);
            ...
            if (!$selfCall) {
                $message = [
                    ...
                    "Args" => isset($trace[$i]["args"])
                        ? FireCake::stringEncode($trace[$i]["args"])
                        : "",
                    "Trace" => FireCake::_escapeTrace(
     ...
     */
    public static function stringEncode(
        $object,
        $objectDepth = 1,
        $arrayDepth = 1
    ) {
        ...
        if (is_object($object)) {
            if ($objectDepth == $_this->options["maxObjectDepth"]) {
                return "** Max Object Depth (" .
                    $_this->options["maxObjectDepth"] .
                    ") **";
            }
            ...
            foreach ($properties as $name => $property) {
                $return[$name] = FireCake::stringEncode(
                    $property,
                    1,
                    $objectDepth + 1
                );
        ...
        if (is_array($object)) {
            if ($arrayDepth == $_this->options["maxArrayDepth"]) {
                return "** Max Array Depth (" .
                    $_this->options["maxArrayDepth"] .
                    ") **";
            }
            foreach ($object as $key => $value) {
                $return[$key] = FireCake::stringEncode(
                    $value,
                    1,
                    $arrayDepth + 1
                );

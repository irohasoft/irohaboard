...
class ToolbarComponent extends Component implements CakeEventListener
{
    ...
    public function __construct(ComponentCollection $collection, $settings = [])
    {
        ...
        parent::__construct(
            $collection,
            array_merge($this->settings, (array) $settings)
        );
    ...
    public function beforeRender(Controller $controller)
    {
        ...
        $this->javascript = array_unique(
            array_merge($this->javascript, $vars["javascript"])
        );
    ...
    {
        if (
            Configure::read("Cache.disable") === true ||
    ...
    protected function _gatherVars(Controller $controller)
    {
        ...
        foreach ($panels as $panelName) {
            ...
            if (!empty($panel->javascript)) {
                $vars["javascript"] = array_merge(
                    $vars["javascript"],
                    (array) $panel->javascript
                );
            }
            if (!empty($panel->css)) {
        ...
        foreach ($panels as $panel) {
            ...
            if (!class_exists($className)) {
                trigger_error(
                    __d(
                        "debug_kit",
                        "Could not load DebugToolbar panel %s",
                        $panel
                    ),
                    E_USER_WARNING
                );
    ...
    protected function _saveState(Controller $controller, $vars)
    {
        ...
        if (isset($vars["variables"]["content"])) {
            ...
            array_walk_recursive($vars["variables"]["content"], function (
                &$item
            ) {
                if (

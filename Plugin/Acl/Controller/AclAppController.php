...
{
    var $components = ["RequestHandler", "Acl.AclManager", "Acl.AclReflector"];
    ...
    private function _check_config()
    {
        ...
        if (!empty($role_model_name)) {
            ...
            if (Configure::read("acl.check_act_as_requester")) {
                ...
                if (
                    !$this->AclManager->check_user_model_acts_as_acl_requester(
                        Configure::read("acl.aro.user.model")
                    )
                ) {
    ...
    {
        if (
            $this->request->params["controller"] != "acos" ||
    ...
    {
        $authorized_role_ids = Configure::read(
            "acl.role.access_plugin_role_ids"
        );
        $authorized_user_ids = Configure::read(
        ...
        if (
            in_array($this->Auth->user($model_role_fk), $authorized_role_ids) ||
            in_array(
                $this->Auth->user($this->_get_user_primary_key_name()),
                $authorized_user_ids
            )
        ) {
    ...
    {
        $aco_path = isset($this->params["named"]["plugin"])
    ...
    function _set_aco_variables()
    {
        $this->set(
            "plugin",
            isset($this->params["named"]["plugin"])
                ? $this->params["named"]["plugin"]
                : ""
        );
    ...
    {
        $this->redirect($this->referer(["action" => "admin_index"]));
    }
}
?>

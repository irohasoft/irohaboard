...
class PrgComponent extends Component
{
    ...
    {
        $this->_defaults = Hash::merge(
            $this->_defaults,
            [
                "commonProcess" => (array) Configure::read(
                    "Search.Prg.commonProcess"
                ),
                "presetForm" => (array) Configure::read(
                    "Search.Prg.presetForm"
                ),
            ],
            $settings
        );
    ...
    public function presetForm($options)
    {
        ...
        foreach ($this->controller->presetVars as $field) {
            ...
            if (
                $paramType === "named" &&
                ($this->encode || !empty($field["encode"]))
            ) {
            ...
            switch ($field["type"]) {
                case "lookup":
                    if (!empty($args[$field["field"]])) {
                        ...
                        $result = $this->controller->{$searchModel}->findById(
                            $args[$field["field"]]
                        );
                        $parsedParams[$field["field"]] = $args[$field["field"]];
                        $parsedParams[$field["formField"]] =
                            $result[$searchModel][$field["modelField"]];
                        $data[$model][$field["field"]] = $args[$field["field"]];
                        $data[$model][$field["formField"]] =
                            $result[$searchModel][$field["modelField"]];
                    }
                    break;
    ...
    public function serializeParams(array &$data)
    {
        foreach ($this->controller->presetVars as $field) {
            ...
            if (
                $this->_defaults["commonProcess"]["paramType"] === "named" &&
                ($this->encode || !empty($field["encode"]))
            ) {
     ...
     * @return void
     */
    public function connectNamed($data = null, array $exclude = [])
    {
        if (!isset($data)) {
     ...
     * @return void
     */
    public function commonProcess($modelName = null, array $options = [])
    {
        $defaults = [
            "excludedParams" => ["page"],
        ];
        ...
        if (!empty($this->controller->request->data)) {
            ...
            if ($valid) {
                ...
                if ($paramType === "named") {
                    ...
                    $this->connectNamed($params, []);
                } else {
                    $searchParams = array_merge(
                        $this->controller->request->query,
                        $searchParams
                    );
                    $searchParams = $this->exclude(
                        $searchParams,
                        $excludedParams
                    );
                    if ($filterEmpty) {
                    ...
                    $this->connectNamed($searchParams, []);
                    $params["?"] = $searchParams;
                }
                ...
                foreach ($allowedParams as $key) {
                    if (isset($this->controller->request->params[$key])) {
                        $params[$key] =
                            $this->controller->request->params[$key];
                    }
                }
                ...
                $this->controller->Session->setFlash(
                    __d("search", "Please correct the errors below.")
                );
            }
        } elseif (
            ($paramType === "named" && !empty($this->controller->passedArgs)) ||
            ($paramType === "querystring" &&
                !empty($this->controller->request->query))
        ) {
            $this->connectNamed($this->controller->passedArgs, []);
            $this->presetForm([
                "model" => $formName,
                "paramType" => $paramType,
            ]);
    ...
    protected function _filter(array $params)
    {
        foreach ($this->controller->presetVars as $key => $presetVar) {
            ...
            if (
                !isset($presetVar["emptyValue"]) ||
                $presetVar["emptyValue"] !== $params[$field]
            ) {
                continue;
            }
            $params[$field] = null;

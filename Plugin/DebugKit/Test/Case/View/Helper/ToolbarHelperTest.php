...
class ToolbarHelperTestCase extends CakeTestCase
{
    ...
    public function testNoCacheConfigPresent()
    {
        ...
        $this->assertFalse(
            $result,
            "Writing to cache succeeded with no cache config %s"
        );

        $result = $this->Toolbar->readCache("test");
        $this->assertFalse(
            $result,
            "Reading cache succeeded with no cache config %s"
        );
    }
    ...
    public function testGetQueryLogs()
    {
        ...
        $result = $this->Toolbar->getQueryLogs($model->useDbConfig, [
            "cache" => false,
        ]);
        $this->assertTrue(is_array($result));
        $this->assertTrue(
            count($result) >= 2,
            "Should be more than 2 queries in the log %s"
        );
        ...
        $this->assertEquals(
            $cached[$model->useDbConfig]["queries"][0],
            $result["queries"][0]
        );
    }
}

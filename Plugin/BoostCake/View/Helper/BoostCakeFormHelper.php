...
class BoostCakeFormHelper extends FormHelper
{
    ...
    protected function _divOptions($options)
    {
        ...
        $default = ["div" => ["class" => null]];
        $options = Hash::merge($default, $options);
        $divOptions = parent::_divOptions($options);
        if ($this->tagIsInvalid() !== false) {
            $divOptions = $this->addClass(
                $divOptions,
                $this->_inputOptions["errorClass"]
            );
        }
        return $divOptions;
    }
    ...
    protected function _getInput($args)
    {
        ...
        if (
            ...
            $this->_inputOptions["checkboxDiv"] !== false
        ) {
            $input = $this->Html->div(
                $this->_inputOptions["checkboxDiv"],
                $input
            );
        }
        ...
        $error = null;
        $errorOptions = $this->_extractOption(
            "error",
            $this->_inputOptions,
            null
        );
        $errorMessage = $this->_extractOption(
            "errorMessage",
            $this->_inputOptions,
            true
        );
    ...
    protected function _selectOptions(
        ...
        $selectOptions = parent::_selectOptions(
            $elements,
            $parents,
            $showParents,
            $attributes
        ...
        if ($attributes["style"] === "checkbox") {
            foreach ($selectOptions as $key => $option) {
                ...
                if (preg_match("/>(<label.*?>)/", $option, $match)) {
                    ...
                    $option =
                        $match[1] . preg_replace("/<label.*?>/", " ", $option);
                    $option = preg_replace(
                        "/(<label.*?)(>)/",
                        '$1 class="' . $class . '"$2',
                        $option

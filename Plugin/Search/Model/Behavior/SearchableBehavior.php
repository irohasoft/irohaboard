...
class SearchableBehavior extends ModelBehavior
{
    ...
    {
        $this->_defaults =
            (array) Configure::read("Search.Searchable") + $this->_defaults;
    ...
    public function setupFilterArgs(Model $Model)
    {
        ...
        foreach ($Model->filterArgs as $key => $val) {
            ...
            if (!isset($val["field"])) {
                $Model->filterArgs[$key]["field"] =
                    $Model->filterArgs[$key]["name"];
            }
            if (!isset($val["type"])) {
    ...
    public function parseCriteria(Model $Model, $data)
    {
        ...
        foreach ($Model->filterArgs as $field) {
            ...
            if (
                !array_key_exists($field["name"], $data) &&
                array_key_exists("defaultValue", $field)
            ) {
                $data[$field["name"]] = $field["defaultValue"];
            }

            if (in_array($field["type"], ["like"])) {
                $this->_addCondLike($Model, $conditions, $data, $field, "LIKE");
            } elseif (in_array($field["type"], ["ilike"])) {
                $this->_addCondLike(
                    $Model,
                    $conditions,
                    $data,
     ...
     */
    public function getQuery(Model $Model, $type = "first", $query = [])
    {
     ...
     */
    public function condIlike(Model $Model, $name, $data, $field = [])
    {
        $conditions = [];
        $field["name"] = $name;
        if (!is_array($data)) {
     ...
     * @return array of conditions
     */
    public function condLike(Model $Model, $name, $data, $field = [])
    {
        $conditions = [];
        $field["name"] = $name;
        if (!is_array($data)) {
     ...
     */
    public function formatLike(Model $Model, $data, $options = [])
    {
     ...
     */
    public function getWildcards(Model $Model, $options = [])
    {
        $options = array_merge($this->settings[$Model->alias], $options);
        return [
            "any" => $options["wildcardAny"],
            "one" => $options["wildcardOne"],
        ];
    }
     ...
     */
    protected function _addCondLike(
        Model $Model,
        &$conditions,
        $data,
        $field,
        ...
        if (!is_array($this->settings[$Model->alias][$settingName])) {
            $this->settings[$Model->alias][$settingName] = [
                "before" => $this->settings[$Model->alias][$settingName],
                "after" => $this->settings[$Model->alias][$settingName],
            ];
        }
        $field = array_merge(
            $this->settings[$Model->alias][$settingName],
        ...
        foreach ($fieldNames as $fieldName) {
            ...
            if (
                !empty($field["connectorAnd"]) ||
                !empty($field["connectorOr"])
            ) {
                $cond[] = $this->_connectedLike(
                    $value,
                    $field,
                    $fieldName,
                    $likeMethod
                ...
                $cond[$fieldName . " " . $likeMethod] =
                    $field["before"] . $value . $field["after"];
            }
        }
        if (count($cond) > 1) {
     ...
     * @return array Conditions
     */
    protected function _connectedLike(
        $value,
        $field,
        $fieldName,
        $likeMethod = "LIKE"
        ...
        foreach ($orValues as $orValue) {
            ...
            foreach ($andValues as $andValue) {
                $and[] = [
                    $fieldName . " " . $likeMethod =>
                        $field["before"] . $andValue . $field["after"],
                ];
            }
    ...
    {
        $fieldNames = (array) $field["field"];
        $fieldValue = isset($data[$field["name"]])
            ? $data[$field["name"]]
            : null;
        ...
        foreach ($fieldNames as $fieldName) {
            ...
            if (
                !is_array($fieldValue) &&
                ($fieldValue === null ||
                    ($fieldValue === "" && empty($field["allowEmpty"])))
            ) {
                continue;
            }
            ...
            if (
                is_array($fieldValue) ||
                (!is_array($fieldValue) && (string) $fieldValue !== "")
            ) {
                $cond[$fieldName] = $fieldValue;
            } elseif (
                isset($data[$field["name"]]) &&
                !empty($field["allowEmpty"])
            ) {
                $schema = $Model->schema($field["name"]);
                if (
                    isset($schema) &&
                    ($schema["default"] !== null || !empty($schema["null"]))
                ) {
     ...
     */
    protected function _addCondExpression(
        Model $Model,
        &$conditions,
        $data,
        $field
        ...
        if (
            ...
            if (
                !empty($conditions[$fieldName]) &&
                is_array($conditions[$fieldName])
            ) {
                $conditions[$fieldName] = array_unique(
                    array_merge([$conditions[$fieldName]], [$fieldValues])
                );
            } else {
     ...
     * @return array of conditions modified by this method
     */
    protected function _addCondSubquery(
        Model $Model,
        &$conditions,
        $data,
        $field
        ...
        if (
            ...
            if (!empty($subquery)) {
                $conditions[] = $Model
                    ->getDataSource()
                    ->expression("$fieldName in ($subquery)");
            }
        }
        return $conditions;
    }
     ...
     * @return string
     */
    protected function _queryGet(Model $Model, $queryData = [])
    {
        /** @var DboSource $db  */

...
class AppFormHelper extends FormHelper
{
     ...
     * @var array
     */
    public $app_tags = ["help", "helpText", "helpList", "example"];
        ...
        foreach ($options as $key => $value) {
            if (in_array($key, $this->app_tags)) {
                if (method_exists($this, $key)) {
                    ...
                    throw new Exception(
                        sprintf("AppFormHelper::%s()がありません。", $key)
                    );
                }
            }
        }
    ...
    {
        $helptext = String::insert('<p class="help-text">:helptext</p>', [
    ...
    {
        $ul = $this->Html->nestedList($options["helpList"], [
            "class" => "help-list",
        ]);
    ...
    {
        $text = String::insert('<p class="example">入力例) :text</p>', [

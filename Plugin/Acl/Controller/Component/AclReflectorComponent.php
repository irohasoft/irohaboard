...
class AclReflectorComponent extends Component
{
    ...
    {
        if (
            strrpos($controller_name, "Controller") !==
            strlen($controller_name) - strlen("Controller")
        ) {
            /*
    ...
    public function get_all_plugins_controllers(
        $filter_default_controller = false
    ) {
        $plugin_paths = $this->get_all_plugins_paths();
        ...
        foreach ($plugin_paths as $plugin_path) {
            ...
            if (!empty($didCD)) {
                ...
                foreach ($files as $fileName) {
                    ...
                    $controller_class_name = Inflector::camelize(
                        substr($file, 0, strlen($file) - strlen(".php"))
                    );

                    if (
                        !$filter_default_controller ||
                        Inflector::camelize($plugin_name) . "Controller" !=
                        ...
                        if (
                            !preg_match(
                                "/^" .
                                    Inflector::camelize($plugin_name) .
                                    "App/",
                                $controller_class_name
                            )
                        ) {
    ...
    public function get_all_plugins_controllers_actions(
        ...
        foreach ($plugin_controllers as $plugin_controller) {
            ...
            if (
                ...
                $ctrl_cleaned_methods = $this->get_controller_actions(
                    $controller_class_name
                );
            ...
            foreach ($files as $fileName) {
                ...
                $controllers[] = [
                    ...
                    "name" => substr(
                        $controller_class_name,
                        0,
                        strlen($controller_class_name) - strlen("Controller")
                    ),
                ];
            }
        }
    ...
    public function get_all_actions()
    {
        ...
        return array_merge(
            $app_controllers_actions,
            $plugins_controllers_actions
        );
    }
    ...
    public function get_controller_actions(
        $controller_classname,
        $filter_base_methods = true
    ) {
        $controller_classname = $this->get_controller_classname(
            $controller_classname
        );

...
class ArosController extends AclAppController
{
    ...
    function admin_check($run = null)
    {
        ...
        $user_display_field = $this->AclManager->set_display_name(
            $user_model_name,
            Configure::read("acl.user.display_name")
        );
        $role_display_field = $this->AclManager->set_display_name(
            $role_model_name,
        ...
        foreach ($roles as $role) {
            ...
            $aro = $this->Aro->find("first", [
                "conditions" => [
                    ...
                    "foreign_key" =>
                        $role[$role_model_name][
                            $this->_get_role_primary_key_name()
                        ],
                ],
            ]);
            ...
            if (count($missing_aros["users"]) > 0) {
                foreach ($missing_aros["users"] as $k => $user) {
                    ...
                    $parent_id = $this->Aro->field("id", [
                        ...
                        "foreign_key" =>
                            $user[$user_model_name][
                                $this->_get_role_foreign_key_name()
                            ],
                    ]);
                    ...
                    if (!empty($parent_id)) {
                        $this->Aro->create([
                            ...
                            "alias" =>
                                $user[$user_model_name][$user_display_field],
                        ]);
    ...
    function admin_users()
    {
        ...
        $this->paginate["order"] = [$user_display_field => "asc"];
        ...
        if (
            isset($this->request->data["User"][$user_display_field]) ||
            $this->Session->check("acl.aros.users.filter")
        ) {
            if (!isset($this->request->data["User"][$user_display_field])) {
                $this->request->data["User"][
                    $user_display_field
                ] = $this->Session->read("acl.aros.users.filter");
            } else {
                $this->Session->write(
                    "acl.aros.users.filter",
                    $this->request->data["User"][$user_display_field]
                );
            }

            $filter = [
                $user_model_name . "." . $user_display_field . " LIKE" =>
                    "%" .
                    $this->request->data["User"][$user_display_field] .
                    "%",
            ];
    ...
    function admin_update_user_role()
    {
        ...
        $data = [
            $user_model_name => [
                $this->_get_user_primary_key_name() => $this->params["named"][
                    "user"
                ],
                $this->_get_role_foreign_key_name() => $this->params["named"][
                    "role"
                ],
            ],
        ];
        ...
        if ($this->{$user_model_name}->save($data, false)) {
            $this->Session->setFlash(
                __d("acl", "The user role has been updated"),
                "flash_message",
                null,
                "plugin_acl"
            );
        ...
        foreach ($actions as $k => $full_action) {
            ...
            if ($controller_name == "App") {
                ...
                if (isset($plugin_name)) {
                    $methods["plugin"][$plugin_name][$controller_name][] = [
                        "name" => $action,
                    ];
                } else {
                    $methods["app"][$controller_name][] = ["name" => $action];
                }
            }
        }
        ...
        foreach ($actions as $full_action) {
            ...
            if ($controller_name != "App") {
                foreach ($roles as $role) {
                    ...
                    if (!empty($aro_node)) {
                        $aco_node = $this->Acl->Aco->node(
                            "controllers/" . $full_action
                        );
                        if (!empty($aco_node)) {
                            $authorized = $this->Acl->check(
                                $role,
                                "controllers/" . $full_action
                            );

                            $permissions[
                                $role[Configure::read("acl.aro.role.model")][
                                    $this->_get_role_primary_key_name()
                                ]
                            ] = $authorized ? 1 : 0;
                        }
    ...
    function admin_user_permissions($user_id = null)
    {
        ...
        if (empty($user_id)) {
            if (
                isset($this->request->data["User"][$user_display_field]) ||
                $this->Session->check("acl.aros.user_permissions.filter")
            ) {
                if (!isset($this->request->data["User"][$user_display_field])) {
                    $this->request->data["User"][
                        $user_display_field
            ...
            if (empty($user_aro)) {
                $display_user = $this->{$user_model_name}->find("first", [
                    "conditions" => [
                        $user_model_name . ".id" => $user_id,
                        "contain" => false,
                        "recursive" => -1,
                    ],
                ]);
                $this->Session->setFlash(
                    sprintf(
                        ...
                        $display_user[$user_model_name][$user_display_field]
                    ),
                    "flash_error",
                    null,
                    "plugin_acl"
                ...
                $count = $this->Aro->Permission->find("count", [
                    "conditions" => ["Aro.id" => $user_aro[0]["Aro"]["id"]],
                ]);
                if ($count != 0) {
    ...
    {
        if ($this->Aro->Permission->deleteAll(["Permission.id > " => 0])) {
            $this->Session->setFlash(
                __d("acl", "The permissions have been cleared"),
                "flash_message",
                null,
                "plugin_acl"
            );
    ...
    function admin_get_role_controller_permission($role_id)
    {
        ...
        if (!empty($aro_node)) {
            $plugin_name = isset($this->params["named"]["plugin"])
                ? $this->params["named"]["plugin"]
                : "";
            $controller_name = $this->params["named"]["controller"];
            $controller_actions = $this->AclReflector->get_controller_actions(
                $controller_name
            );

            $role_controller_permissions = [];

            foreach ($controller_actions as $action_name) {
                $aco_path = $plugin_name;
                $aco_path .= empty($aco_path)
                    ? $controller_name
                    : "/" . $controller_name;
    ...
    function admin_grant_role_permission($role_id)
    {
        ...
        if (!empty($aro_node)) {
            if (
                !$this->AclManager->save_permission(
                    $aro_node,
                    $aco_path,
                    "grant"
                )

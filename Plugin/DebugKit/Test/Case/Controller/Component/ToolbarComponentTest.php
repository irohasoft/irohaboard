...
class ToolbarComponentTestCase extends CakeTestCase
{
    ...
    protected function _loadController($settings = [])
    {
        ...
        $this->Controller->components = [
            "Toolbar" => $settings + ["className" => "TestToolbar"],
        ];
        $this->Controller->constructClasses();
        $this->Controller->Components->trigger("initialize", [
            $this->Controller,
        ]);
    ...
    public function testLoadPanels()
    {
        ...
        $this->assertInstanceOf(
            "SessionPanel",
            $this->Controller->Toolbar->panels["session"]
        );
        $this->assertInstanceOf(
            "RequestPanel",
            $this->Controller->Toolbar->panels["request"]
        );

        $this->Controller->Toolbar->loadPanels(["history"], ["history" => 10]);
        $this->assertEquals(
            $this->Controller->Toolbar->panels["history"]->history,
            10
        );
    }
    ...
    {
        $this->Controller->Toolbar->loadPanels([
            "randomNonExisting",
            "request",
        ]);
    }
    ...
    public function testLibPanels()
    {
        ...
        $this->assertInstanceOf(
            "TestPanel",
            $this->Controller->Toolbar->panels["test"]
        );
    }
    ...
    public function testInitializeCustomPanelsWithDefaults()
    {
        ...
        $this->assertEquals(
            $expected,
            array_keys($this->Controller->Toolbar->panels)
        );
    }
    ...
    public function testStateSaving()
    {
        ...
        $this->assertEquals(
            $result[0]["variables"]["content"]["test"],
            "testing"
        );
    ...
    public function testBeforeRender()
    {
        ...
        $this->assertTrue(
            isset($this->Controller->helpers["DebugKit.Toolbar"])
        );
        $this->assertEquals(
            $this->Controller->helpers["DebugKit.Toolbar"]["output"],
            "DebugKit.HtmlToolbar"
        );
        $this->assertEquals(
            $this->Controller->helpers["DebugKit.Toolbar"]["cacheConfig"],
            "debug_kit"
        );
        $this->assertTrue(
            isset($this->Controller->helpers["DebugKit.Toolbar"]["cacheKey"])
        );

        $this->assertTrue(
            isset($this->Controller->viewVars["debugToolbarPanels"])
        );
    ...
    public function testBeforeRedirect()
    {
        ...
        $this->Controller->Toolbar->beforeRedirect(
            $this->Controller,
            "/another/url"
        );
    ...
    public function testHistoryUrlGenerationWithPrefixes()
    {
        ...
        $result = $this->Controller->Toolbar->panels["history"]->beforeRender(
            $this->Controller
        );
        $expected = [
            "plugin" => "debug_kit",
    ...
    public function testNoRequestActionInterference()
    {
        ...
        $result = $this->Controller->requestAction(
            "/debug_kit_test/request_action_return",
            ["return"]
        );
        $this->assertEquals($result, "I am some value from requestAction.");

        $result = $this->Controller->requestAction(
            "/debug_kit_test/request_action_render",
            ["return"]
        );

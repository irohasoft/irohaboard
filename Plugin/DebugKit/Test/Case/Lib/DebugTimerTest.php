...
class DebugTimerTest extends CakeTestCase
{
    ...
    public function testAnonymousTimers()
    {
        ...
        $this->assertTrue($timer["time"] > 0.002);
    ...
    public function testNestedAnonymousTimers()
    {
        ...
        $this->assertTrue(
            isset($timers[$file . " line " . $firstTimerLine]),
            "first timer is not set %s"
        );
        $this->assertTrue(
            isset($timers[$file . " line " . $secondTimerLine]),
            "second timer is not set %s"
        );
    ...
    public function testRepeatTimers()
    {
        ...
        $this->assertTrue(
            $timers["my timer"]["time"] > $timers["my timer #2"]["time"],
            "timer 2 is longer? %s"
        );
        $this->assertEquals(
            "This is the first call",
            $timers["my timer"]["message"]
        );
        $this->assertEquals(
            "This is the second call #2",
            $timers["my timer #2"]["message"]
        );
    }

...
class RecordsController extends AppController
{
    ...
    public function admin_index()
    {
        ...
        $name = isset($this->request->query["name"])
            ...
            : "";
        $content_category = isset($this->request->query["content_category"])
            ? $this->request->query["content_category"]
            : "";
        $contenttitle = isset($this->request->query["contenttitle"])
            ? $this->request->query["contenttitle"]
            : "";
        ...
        if (@$this->request->query["cmd"] == "csv") {
            ...
            header(
                'Content-Disposition: attachment; filename="quiz_records.csv"'
            );
            ...
            foreach ($rows as $row) {
                $row = [
                    ...
                    Configure::read(
                        "record_result." . $row["Record"]["is_passed"]
                    ),
                    Configure::read(
                        "record_understanding." .
                            $row["Record"]["understanding"]
                    ),
